...
Write-Host "------------------------------------------"
$HyperVState = Get-WindowsOptionalFeature -Online -FeatureName *hyper-v* | select DisplayName, FeatureName, State
$HyperVState | ft * -AutoSize
...
Write-Host
	...
	else { $RequiredVLANs = $Null }
	# Make sure we have an array!
	If ( $RequiredVLANs -eq $Null ) { $RequiredVLANs = @() }
	elseif ($RequiredVLANs.GetType().Name -eq "PSCustomObject") { $RequiredVLANs = @( $RequiredVLANs ) }
	...
	ForEach ($VLAN in $RequiredVLANs) {
        If ( ![string]::IsNullOrEmpty($VLAN.Routes) ) {
            ...
            ForEach ($Network in $Networks) {
                ...
                Catch { Write-Warning $( $VLAN.Routes + " ignored for VLan id " + $VLAN.ID + ".")
                        $IgnoreNetworks = $True
                  }
                Finally {
			...
			If ($IgnoreNetworks) { Write-Host "" }	# Only if warnings were issued
			else { $VLAN.Routing = $Routing }		# Only if all routes appear valid (which may or may not be true ...)
      }
    }

# Get all virtual switches
[System.Object] $AllPhysicalPorts = Get-NetAdapter | Get-NetAdapterBinding | Where { ($_.Enabled -eq $true) -and ($_.ComponentID -eq "vms_pp") }
...
"@
	...
	$AllPhysicalPorts | ft @{Label="Nom du port physique"; Expression={$_.Name}}, InterfaceDescription, Description -AutoSize
	If ($(Read-Host "Enter 'Yes' to proceed, anything else to continue").tolower().StartsWith('yes')) {
		...
		Write-Host "------------------------"
		Try { $DefaultRoutes = Get-NetRoute -DestinationPrefix "0.0.0.0/0" -PolicyStore ActiveStore -ErrorAction Stop
			  $DefaultRoutes | ft InterfaceAlias, InterfaceIndex, NextHop }
		Catch { $DefaultRoutes = $Null
				Write-Host "None!" }

		ForEach ($Gateway in $DefaultRoutes.NextHop) { If ( $(Test-NetConnection -ComputerName $Gateway -InformationLevel Quiet)) { Write-Host $Gateway, "is reachable." } }
		...
		ForEach ($PhysicalPort in $AllPhysicalPorts) {
			...
			$Adapter | ft Name, PhysicalMediaType, LinkSpeed, MacAddress

			$Switch = Get-VMSwitch -SwitchType External | Where { $_.NetAdapterInterfaceDescription -eq $PhysicalPort.InterfaceDescription }
			$Switch | ft Name, SwitchType, NetAdapterInterfaceDescription


			$DefinedVLANs = Get-VMNetworkAdapterVLAN -ManagementOS | Where { $_.ParentAdapter.SwitchName -eq $Switch.Name }
...
If ($RequiredVLANs -ne $Null) {
	...
	# Get THE virtual switch (Only one switch is supported in this script).
	  ...
	  ElseIf ($Switch.Count -eq 0)
		{
		  ...
		  [System.Object] $Adapter = (Get-NetAdapter | Where { $_.Status -eq "UP" -and $_.PhysicalMediaType -eq "802.3" })
	  ...
	  # Get the status of the switch on this system
		$ManagedVLANs = @()
		ForEach ( $Port in $(Get-VMNetworkAdapterVLAN -ManagementOS | Where { $_.ParentAdapter.SwitchName -eq $Switch.Name }) ){
	  ...
	  # Add missing VLANs:
		ForEach ($VLAN in $RequiredVLANs) {
			If (-not ($VLAN.ID -in $ManagedVLANs.VLANId)) {
				...
				If ($VLAN.ID -eq 0) { $ThisOne | Set-VMNetworkAdapterVLAN -Untagged }
				Else { $ThisOne	| Set-VMNetworkAdapterVLAN -Access -VLANId $VLAN.ID }
				Write-Host "Adding: ", $VLAN.ID, $VLAN.Name
			  }
			Else {	$Port = Get-VMNetworkAdapterVLAN -ManagementOS `
							| Where { ($_.ParentAdapter.SwitchName -eq $Switch.Name ) -and `
	  ...
	  # Display virtual switch status
		...
		ForEach ( $Port in $(Get-VMNetworkAdapterVLAN -ManagementOS | Where { $_.ParentAdapter.SwitchName -eq $Switch.Name }) ){
			$ManagedVLANs += New-Object -Type PSOBJECT -Property @{
					...
					VLANId = $Port.AccessVlanId
					PortName = $Port.ParentAdapter.Name
					MACAddress = $(Get-VMNetworkAdapter -ManagementOS | Where { $_.Name -eq $Port.ParentAdapter.Name } ).MACAddress -replace '..(?!$)', '$&-'
			}
		  }
		$ManagedVLANs | ft * -Autosize

		# Disable all VLANs
		ForEach ($VLAN in $ManagedVLANs) {
			If ($VLAN.VLANId -ne 0) {
				$Decorated = Get-NetAdapter | Where { $_.MACAddress -eq $VLAN.MACAddress } `
				...
				$Configuration = $RequiredVLANs | Where { $_.ID -eq $VLAN.VLANId }
				ForEach ($Route in $Configuration.Routing) {
					$NewRoute = New-NetRoute -DestinationPrefix $Route.Prefix -NextHop $Route.Gateway -InterfaceIndex $Decorated.ifIndex
				 }
			...
			Else {
				...
				$Decorated = Get-NetAdapter | Where { ($_.MACAddress -eq $VLAN.MACAddress) -and $_.InterfaceDescription.Contains("Hyper-V Virtual Ethernet Adapter") } `
	  ...
	  Write-Host
	  If ($Adapter.Name -ne $DefaultPhysicalPortName) { Rename-NetAdapter -InputObject $Adapter -NewName $DefaultPhysicalPortName }
...
Do {
	Try {
		...
		Get-NetRoute -IncludeAllCompartments -ErrorAction Stop `
			| Where-Object -FilterScript { $_.ValidLifetime -ne ([TimeSpan]::MaxValue) } | fT ifIndex, DestinationPrefix, NextHop, InterfaceAlias, ValidLifetime -AutoSize
		$Done = $True
	  }
	Catch {
...
Write-Host
Write-Host "Current default route(s):"
Try { $(Find-NetRoute -RemoteIPAddress 0.0.0.0 -ErrorAction Stop )[1] | fl InterfaceAlias, InterfaceMetric }
Catch { Write-Warning "None! You may have to wait a few moments ;-)" }
